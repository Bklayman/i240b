...
static void outLineLengths(std::ostream& out){
  for(long unsigned int i = 0; i < sizeof(points)/sizeof(points[0]); i+=2){
    Line curLine = Line(points[i], points[i + 1]);
    out << curLine.toString() << "\n";
  }
}
