...
go(SortP sort, std::string filePath, bool isVerbose, int nRead)
{
  ...
  result.push_back(ops.compareCount);
  result.push_back(ops.swapCount);
  return result;
}
...
main(int argc, char* argv[]) {
  ...
  if (sizeArgsIndex == argc) {
    ...
    std::cout << "n\tcompared\tswaps" << std::endl;
    for (int i = sizeArgsIndex; i < argc; ++i) {
      int nRead = atoi(argv[i]);
      std::vector<int> counts = go(sort, filePath, isVerbose, nRead);
      std::cout << nRead << "\t" << counts[0] << "\t" << counts[1] << std::endl;
    }
  }
  }
